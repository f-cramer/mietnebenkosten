...
class DevelopmentConfiguration {
    ...
    fun initializer(
        ...
        val user = userService.saveUser(User("test", passwordEncoder.encode("user123"), mutableListOf()))

        val rentalComplex = rentalComplexService.createRentalComplex(RentalComplexForm("Kastanienallee 39"), user)
        val secondaryRentalComplex = rentalComplexService.createRentalComplex(RentalComplexForm("Hallesches Ufer 31"), user)
        ...
        landlordService.createLandlord(LandlordForm("Torsten", "Schweitzer", "Hans-Grade-Allee", 86, "24875", "Havetoftloit", null, "DE81 5001 0517 8228 8538 63", Year.of(2014), null), secondaryRentalComplex)

        val erdgeschossSecondary = flatService.createFlat(FlatForm("Erdgeschoss", 110), secondaryRentalComplex)
        val og1Secondary = flatService.createFlat(FlatForm("1. Obergeschoss", 110, 1), secondaryRentalComplex)
        val dachgeschossSecondary = flatService.createFlat(FlatForm("Dachgeschoss", 80, 2), secondaryRentalComplex)

        val tenant1Secondary = tenantService.createTenant(TenantForm("Leonie", "Dehmel", "Wittrockstraße", 24, "34121", "Kassel", gender = Gender.FEMALE, formOfAddress = FormOfAddress.INFORMAL, hidden = false), secondaryRentalComplex)
        val tenant2Secondary = tenantService.createTenant(TenantForm("Rainer", "Utz", "Alte Kollaustrasse", 44, "22529", "Hamburg", gender = Gender.MALE, formOfAddress = FormOfAddress.FORMAL, hidden = false), secondaryRentalComplex)
        val tenant3Secondary = tenantService.createTenant(TenantForm("Leonard", "Jüngling", "Große Bockenheimer Straße", 52, "60313", "Frankfurt am Main", gender = Gender.MALE, formOfAddress = FormOfAddress.INFORMAL, hidden = false), secondaryRentalComplex)

        contractService.createContract(ContractForm(erdgeschossSecondary.id, tenant1Secondary.id, 2, LocalDate.of(year.value, Month.JANUARY, 1)))
        contractService.createContract(ContractForm(og1Secondary.id, tenant2Secondary.id, 1, LocalDate.of(year.value, Month.JANUARY, 1)))
        contractService.createContract(ContractForm(dachgeschossSecondary.id, tenant3Secondary.id, 1, LocalDate.of(year.value, Month.JANUARY, 1)))

        invoiceService.createInvoice(generalInvoice("Streusalz", 5.euros(), SplitAlgorithmType.ByArea, year, 1), secondaryRentalComplex)
        invoiceService.createInvoice(generalInvoice("Benzin Rasenmäher", 17.euros() + 25.cents(), SplitAlgorithmType.ByArea, year, 2), secondaryRentalComplex)
        invoiceService.createInvoice(generalInvoice("Allgemeinstrom", 330.euros(), SplitAlgorithmType.ByArea, year, 3), secondaryRentalComplex)
    }
}
